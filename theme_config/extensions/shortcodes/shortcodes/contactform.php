...
function tf_contactform_shortcode($atts){
    ...
    if($form_exists){
        ...
        if(urldecode($form['header_message'])!='&nbsp;' && urldecode($form['header_message'])!='')$out .='<h2 id="header_message">'.urldecode($form['header_message']).'</h2>';
        ...
        if(urldecode($form['reset_button'])!='&nbsp;' && urldecode($form['reset_button'])!='') $out.='<span class="reset-link"><a href="#" onclick="resetFields(this,event)" style="float: left;">'.urldecode($form['reset_button']).'</a></span><br>';
        $out.='<a href="#" id="sending" class="btn-submit" style="display:none"><span>Sending ...</span></a>
        <input type="submit" id="send_form" class="btn-submit" name="submit" title="Submit message" value="'. urldecode($form['submit_mess']).'" />
...
function radio($input,$form_input){
    ...
    $output="<div class='row alignleft input_styled ".$form_input['classes']."' style='width:".($form_input['width']-5)."%; margin-top:-2px;".$form_input['floating']."'>
                <label class='label_title' for='" .TF_THEME_PREFIX.'_'.trim($form_input['shortcode']). "'>".$form_input['label_text']."</label>";

    if(is_array($form_input['options'])){
        foreach ($form_input['options'] as $key => $option) {
            $output .= '<div class="rowRadio"><input '.$checked.' id="' .TF_THEME_PREFIX.'_'. trim($form_input['shortcode']). '_'.$key.'"  type="radio" name="' .TF_THEME_PREFIX.'_'. trim($form_input['shortcode']). '"  value="' .$option. '" /><label class="" for="' .TF_THEME_PREFIX.'_'. trim($form_input['shortcode']). '_'.$key.'">' . $option . '</label></div>';
            $checked='';
        }
    }
...
function checkbox($input,$form_input){
    ...
    $output = "<div class='row alignleft input_styled checklist ".$form_input['classes']."' style='width:".($form_input['width']-5)."%;".$form_input['floating']."'><div class='rowCheckbox'><div class='custom-checkbox'>
...
function select($input,$form_input){
    ...
    $out = "<div class='row field_text alignleft ".$form_input['classes']."' style='".$form_input['floating']."'>
                <label class='label_title' for='" .TF_THEME_PREFIX."_".trim($form_input['shortcode']). "'>".$form_input['label_text']."</label>";
    $out .= "<select  style='width:".($form_input['ewidthpx']+$select)."px;' class='".$input['properties']['class']."' name='" .TF_THEME_PREFIX."_".trim($form_input['shortcode']). "' id='" .trim($input['id']). "'>";
    if(is_array($form_input['options'])){
        foreach ($form_input['options'] as $key => $option) {
